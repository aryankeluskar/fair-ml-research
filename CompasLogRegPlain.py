...
for repetition in range(15):
    ...
    def heart_disease_data():
        data = pd.read_csv("./compasPreprocessed2.csv")
        # drop rows with missing values
        data = data.dropna()
        # balance data
        grouped = data.groupby('ScoreText')
        data = grouped.apply(lambda x: x.sample(grouped.size().min(), random_state=73).reset_index(drop=True))
        # extract labels
        y = torch.tensor(data["ScoreText"].values).float().unsqueeze(1)
        data = data.drop("ScoreText", axis='columns')
        # standardize data
        data = (data - data.mean()) / (data.std() + 1e-8)

        
        # store data in csv called compasMidPreprocessed.csv
        data.to_csv('compasMidPreprocessed.csv', index=False)
    ...
    with open("PlainCompas.txt", "a") as file:
